...
const Map: React.FC<RouteComponentProps<{id:string}>> = (props) => {
  ...
  const getEntityByBusinessType = async (businessType: string) => {
    ...
    const entityData: any = await Promise.all(
      data.entities.map(async (entity: { name: string; id: string }) => {
        const entityPropertyResponse = await fetch(
          `${process.env.REACT_APP_DATABASE_URL}/entity_property/${entity.id}.json`
        );
    ...
    const entityGeoData: any = await Promise.all(
      filteredEntityData.map(async (entity: { id: string }) => {
        const geoData = await fetchIndividualEntityData(
          "geojson",
          entity.id,
          "geojson"
        );
  ...
  const handleFeaturedLocations = async () => {
    ...
    const featuredEmissionsData: any = await Promise.all(
      featured.map((entityID) => {
        return fetchIndividualEntityData("entity_property", entityID, "json");
      })
    );

    const featuredGeoData: any = await Promise.all(
      featured.map((entityID) => {
        return fetchIndividualEntityData("geojson", entityID, "geojson");
      })
    );
  ...
  const handleIndividualEntity = async () => {
    ...
    if (!fetchedGeoData) {
      ...
      const fetchedEmissionsData: any[] = await fetchIndividualEntityData(
        "entity_property",
        entityID,
        "json"
      );
      ...
      if (!fetchedEmissionsData) {
        ...
        const featuredEmissionsData: any = await Promise.all(
          featured.map((entityID) => {
            return fetchIndividualEntityData(
              "entity_property",
              entityID,
              "json"
            );
  ...
  useEffect(() => {
    ...
    if (location.pathname.substring(0, 14) !== "/business-type") {
      ...
      setEntitiesByBusinessType([]);
      setEmissionsData([]);
      const businessType = location.pathname.substring(
        15,
  ...
  useEffect(() => {
    if (emissionsData.length > 0) {
      ...
      if (navHistory === undefined) {
        setCookie("history", [
          { name: emissionsData[0].name, id: emissionsData[0].id },
        ]);
      } else {
        const matchEntity = navHistory.find(
          (entity: any) => entity.name === emissionsData[0].name
        );
